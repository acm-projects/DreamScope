...
app.post("/api/dreamPosts", async (req, res) => {
  try {
    ...
    const combinedDreamText =
      type === "Fragmented" ? dreamFragments.join("\n") : dreamText;
...
app.get("/api/dreamPosts/user/:userId", async (req, res) => {
  try {
    ...
    if (!dreamPosts || dreamPosts.length === 0) {
      return res
        .status(404)
        .json({ error: "No dream posts found for this user." });
    }
          ...
          return {
            ...
            metadata: {
              ...
              compressionRatio: vis.compressedSize
                ? (
                    ((vis.originalSize - vis.compressedSize) /
                      vis.originalSize) *
                    100
                  ).toFixed(2)
                : null,
...
app.delete("/api/dreamPosts/:postId", async (req, res) => {
  try {
    const deletedDreamPost = await DreamPost.findByIdAndDelete(
      req.params.postId
    );
    if (!deletedDreamPost) {
      return res.status(404).json({ error: "Dream post not found." });
...
app.get("/api/dreamPosts/user/:userId/date/:date/postId/:postId",
  async (req, res) => {
    try {
      const { userId, date } = req.params;
      const parsedDate = new Date(date);
