...
const getSubpromptsFromChatGPT = async (rawPrompt) => {
    const chatOptions = {
        ...
        body: JSON.stringify({
            ...
            messages: [
                {
                    role: "system",
                    content: "Split the input text into 4 separate paragraphs of roughly equal length. No more than 30 words in each paragraph. Each paragraph should be delimited by -- and be descriptive enough to generate a DALL·E 3 image from. Keep each one imaginative and vivid with a focus on the background setting and characters."
                },
                {
                    role: "user",
                    content: rawPrompt
    ...
    try {
        const res = await fetch("https://api.openai.com/v1/chat/completions", chatOptions);
        const data = await res.json();
        const fullSplitPrompt = data.choices[0].message.content;
        const subprompts = fullSplitPrompt.split("--").map(p => p.trim()).filter(p => p.length > 0);
...
const generateImageFromPrompt = async (promptText, index) => {
    ...
    try {
        const res = await fetch("https://api.openai.com/v1/images/generations", imageOptions);
        const data = await res.json();
